...
class Node(gossip_pb2_grpc.GossipServiceServicer):
    ...
    def SendMessage(self, request, context):
        ...
        if message.endswith("-0") and sender_id == self.host:

            self.received_messages.add(message)

            # Getting neighbors info
            self.get_neighbours()

            log_message = (f"Pod preparation initiated by {self.hostname} ({self.host}) at "
            ...
            # Distribute message to neighbors  (that is  not receiving the message yet)
            self.gossip_message(message, sender_id)

        elif message.endswith("-0") and sender_id != self.host:

            self.received_messages.add(message)
            propagation_time = (received_timestamp - request.timestamp) / 1e6
            log_message = (f"Preparing pod ({self.hostname}({self.host}) and received: '{message}' from {sender_id}"
        ...
        # Check for gossip message initiation and set the initial timestamp
        elif sender_id == self.host:
            self.received_messages.add(message)
