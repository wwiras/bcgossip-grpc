...
def access_pod_and_initiate_gossip(pod_name, replicas, iteration):
    """
    ...
    try:
        ...
        session.stdin.write(f'python initiate.py --message cubaan{replicas}-{iteration}\n')
        session.stdin.flush()
...
def main(num_tests):
    ...
    if wait_for_pods_to_be_ready(namespace='default', expected_pods=replicas, timeout=300):
        for i in range(1, num_tests + 1):
            ...
            if access_pod_and_initiate_gossip(pod_name, replicas, i):
...
if __name__ == '__main__':
    if len(sys.argv) != 2:
        print("Usage: python bcgossip1.py <number_of_tests>")
        sys.exit(1)
