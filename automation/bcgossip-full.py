...
def access_pod_and_initiate_gossip(test_unique_id, pod_name, replicas, unique_id, iteration):
    try:
        session = subprocess.Popen(['kubectl', 'exec', '-it', pod_name, '--', 'sh'], stdin=subprocess.PIPE,
                                   stdout=subprocess.PIPE, stderr=subprocess.PIPE, text=True)
        message = f'{test_unique_id}-{unique_id}-cubaan{replicas}-{iteration}'
...
def main(num_tests, deployment_folder):
    ...
    test_unique_id = str(uuid.uuid4())[:3]  # Generate a unique test-wide ID
    ...
    for deployment_file in deployment_files:
        ...
        if wait_for_pods_to_be_ready(namespace='default', expected_pods=replicas, timeout=300):
            for i in range(1, num_tests + 1):
                unique_id = str(uuid.uuid4())[:5]  # Generate a unique ID for each test iteration
                pod_name = select_random_pod()
                print(f"Selected pod for test {i}: {pod_name}", flush=True)
                if access_pod_and_initiate_gossip(test_unique_id, pod_name, replicas, unique_id, i):
...
if __name__ == '__main__':
    if len(sys.argv) != 3:
        print("Usage: python bcgossip-all.py <number_of_tests> <deployment_folder>")
        sys.exit(1)
