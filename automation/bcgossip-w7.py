    ...
    while time.time() - start_time < timeout:
        try:
            ...
            if running_pods >= expected_pods:
                ...
                # --- Apply tc rules here ---
                # 1. Get Pod IPs
                pod_ips = subprocess.run(["kubectl", "get", "pods", "-n", namespace, "-o", "jsonpath='{.items[*].status.podIP}'"],
                ...
                for i in range(1, num_tests + 1):  # Assuming 'num_tests' is defined in the outer scope
                    pod_name = "gossip-statefulset-0"
                    # pod_name = select_random_pod()
                    print(f"Selected pod for test {i}: {pod_name}", flush=True)
                    if access_pod_and_initiate_gossip(pod_name, num_nodes, speed, unique_id, i):  # Assuming these variables are defined
                        print(f"Test {i} complete for {deployment_file}.", flush=True)  # Assuming 'deployment_file' is defined
                    else:
...
def apply_tc_rules(pod_name, pod_ips, speed):
    ...
    # Logic to determine neighboring pods (adjust as needed)
    # In this simplified version, we assume all pods are neighbors
    neighbor_pods = [ip for ip in pod_ips if ip != pod_name]

    for neighbor_ip in neighbor_pods:
        tc_command = f"tc qdisc add dev eth0 root handle 1: htb default 12; \
        ...
        try:
            subprocess.run(["kubectl", "exec", "-it", pod_name, "--", "bash", "-c", f"sudo {tc_command}"], check=True)
